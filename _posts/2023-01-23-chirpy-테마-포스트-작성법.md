...
댓글창의 전역 스위치는 `_config.yml`{: .filepath} 파일의 `comments.active`이다. 댓글 툴을 선택하면 되는데, 이 블로그는 disqus를 활용했다. (comment.active에 "disqus"라고 입력해주면 됨) 해당 툴의 홈페이지에서 회원가입하고 하라는대로 하면 된다.

마찬가지로 특정 포스트에서만 기능을 선택하고 싶다면 **Front Matter** 에 다음을 추가한다:
